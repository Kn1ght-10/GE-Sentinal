...
**Market-manipulation & RMT surveillance for the Old School RuneScape Grand Exchange** — an end-to-end anomaly-detection system over a real exchange's 5-minute price feed: ingestion → storage → features → weak supervision → 5-detector ensemble → walk-forward evaluation → alert queue → retrieval-grounded analyst memos → API + dashboard.

The Grand Exchange is a genuine electronic marketplace (~3,700 instruments, real order flow, public 5-minute data) that suffers real financial-crime analogues: coordinated pump-and-dumps by "merch clans" and real-money-trading (RMT) value transfers disguised as trades. Jagex bans both sides of RMT; the OSRS Wiki's own API FAQ notes that suspected RMT shows up as **low-volume items trading at extraordinary prices** — which is exactly the signature this system hunts.

![Injected pump & dump, detected with zero delay](assets/fig_event_anatomy.png)

## Measured results

Validation on **synthetic injected events** in a 30% held-out time window (40 items × 21 days = 241,920 five-minute records + 95 genuine API records). Models never see ground truth — they train on weak labels only; truth exists solely for scoring. Full protocol below.
...
Generalization check: re-running the identical pipeline on an unseen simulator seed gives precision@5/@10 = 1.00/1.00, precision@25 = 0.64 — no seed-specific tuning.

![Evaluation summary](assets/fig_eval_summary.png)

## What's real vs. synthetic (read this)

Honesty is a design constraint here, not a disclaimer:

- **Real**: the ingestion layer speaks to the live [OSRS Wiki Real-time Prices API](https://prices.runescape.wiki/api/v1/osrs) (with the mandatory descriptive User-Agent); the repo ships a committed excerpt of **genuine API data** (95 five-minute records for Abyssal whip, including one genuine missing bucket that the gap-report surfaces rather than papering over). Real rows are tagged `seed_real`/`live` in the DB.
- **Synthetic**: detection metrics require ground truth, and no public dataset labels GE manipulation. So evaluation runs on a calibrated simulator (mean-reverting log-price + seasonal Poisson volume) with **injected, labeled events**: pump-and-dumps (ramp + retrace on 8–20× volume), RMT spikes (1–3 buckets at 8–18× median price on ≤3 units), and *benign* patch shocks (permanent level shifts — the look-alike the system must NOT flag). Synthetic rows are tagged `source="synth"`.
- **Never crossed**: models see weak labels only; truth labels touch nothing but the evaluator. Real items are scored but excluded from metrics (no truth exists for them). Every reported number is therefore "validation on synthetic injected events" — a detection-capability claim, not a claim about live-market prevalence.

![Real API data with a genuine gap](assets/fig_real_whip.png)

## Architecture

```
OSRS Wiki API ──collector──▶ SQLite/Postgres(Timescale) ──▶ features (robust rolling
 (live, UA req'd)  (cron/Prefect)  items·prices_5m·alerts      median/MAD z-scores)
                                                                      │
        synthetic market w/ injected truth ──────────────────────────┤
                                                                      ▼
   weak labels (labeling functions, no hand labels) ──▶ 5-detector ensemble
     lf_pump · lf_rmt · lf_quiet · lf_patch              rules │ stats │ IsolationForest
   ...
   precision curve · recall · delay · AUC                      │            Streamlit
                                                               ▼
...
1. **Robust z-scores with denominator floors.** Illiquid items have long runs of zero volume/identical prices, collapsing the MAD to 0 and turning any tick into an "infinite" anomaly — the classic illiquid-surveillance false-positive source. Denominator = max(MAD, 0.5·rolling σ, absolute floor).
2. **The supervised model is a re-ranker, not a gate.** The weak-label GBM's probabilities saturate (257 noise episodes all scoring exactly at the ceiling in the first full run — zero ranking power). It now multiplies the unsupervised ensemble by 0.75–1.25× and can never originate an alert alone. Alert volume fell 46 → 6.5/day; recall stayed 1.00.
3. **Median-filtered seasonal baseline.** A seasonal-naive forecaster using raw lag-288 echoes any 1-bucket spike exactly one day later (phantom alerts at +288 buckets after every RMT print — visible in the diagnostics, ranks 6–10). The daily-lag reference is now a 13-bucket centered median: wide enough to outvote a spike plus its forward-fill smear (≤9 buckets), narrow enough that genuine multi-hour pumps (≥18 buckets) pass through.
4. **Patch suppression + negative labeling.** Announced update windows damp scores ×0.3 and vote negative in weak labeling — that's why benign patch shocks produce zero top-25 false alarms while remaining visible in the alert tail.
...
.github/workflows/    ci.yml (ruff+pytest) · collect.yml (scheduled live collection)
...
- Diagnosed and fixed three production-grade failure modes — zero-MAD blowups on illiquid series, a saturated classifier gating alerts, and a seasonal forecaster echoing spikes one day later — **cutting alert volume 86% (46 → 6.5/day) at unchanged 1.00 recall**, verified on an unseen simulator seed.
...
MIT © 2026 Pranav Tiwari. Not affiliated with Jagex or the OSRS Wiki; respect the API's [acceptable-use policy](https://prices.runescape.wiki/) — descriptive User-Agent, no hammering.
